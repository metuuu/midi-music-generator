...
**And two benches, which are worth more than they look.** `/looks` draws every hair style, every accessory, every accessory crossed with every hair, every fabric and every genre's wardrobe through the real casting path; `/models` is the same for instruments. They exist because sixteen hair styles, twenty accessories and fifteen fabrics had been written, verified against geometric assertions, shipped, and never actually looked at — the only way to see one was to start a concert and hope the camera cut to it. The costume bench **found eighteen faults on its first run**, including a hood whose face-hole was over the right ear and a fabric that exists to look expensive being the darkest thing on stage. The lesson is one line and it generalises past this repo: an assertion catches what you thought to assert, and a bench catches the rest.
