...
**The third finding is the one worth acting on and is left open.** `DrumTrack.voiceEffects` is read by `render/strudel.ts` and **written by nothing** — no genre, no era, no style, and `generate/song.ts` never sets it. Measured at **0 of 380 songs across all nineteen genres**. Four separate files cite it as *"the field that exists for exactly this"* — pop's era table, pop's `stadium`, rock's era table and rock's own style — and none of them can reach it, because there is no `voiceEffects` on `Style`, `EraProfile` or `Genre` for an author to fill in. The home exists and has no door. That is §7's thesis in its purest form and it is a different job from this one: even wired up it would not deliver the gate, because the gate is refused above for reasons that have nothing to do with authoring.
...
**The top four entries on this list and both staging leftovers are gone** — §2.2, §3.16 and the two rooms were the adoption wave in §7, §1.1 closed before it, §3.15 closed after, and §3.17 closed after that. **What is left is a tail and nothing else**: no entry below was reported by more than one genre, and none of them removes a technique. That is a different state from the one this section has described at every previous writing, where the head of the list was always something a genre could not play.

1. **§3.19** — compound time, which may be a documentation gap rather than a mechanism one: rnb's `gospelsoul` got 6/8 honestly with `beatsPerBar: 3` and `groups: [6, 6]`, while pop and `doowop` both approximated 12/8 as swing over 4/4 and said so. Somebody should try the working shape on the approximating styles before anything is built.
2. **§3.20** — no object for a person holding a microphone, distinct from §2.2 now that the vocal *group* exists: this one wants the act rather than the instrument.
3. **§1.3** — bass riff span, the one ceiling everybody has learned to write around. Notably the glide adoption did *not* make it worse, because `generateBass` folds a destination into the same span reduce and all three authors kept their destinations inside tones the figure already struck.

**`DrumTrack.voiceEffects` has a home and no door, and that is now the clearest §7 case in the project.** Read by `render/strudel.ts`, written by nothing, **0 of 380 songs across all nineteen genres** — while four files in two genres cite it as the field that exists for the gated snare. There is no `voiceEffects` on `Style`, `EraProfile` or `Genre`, so no author can reach it. Wiring it would not deliver the gate, which §6 refuses for unrelated reasons; what it would deliver is the per-voice treatment those four comments already describe and cannot write. It wants an author with a table, not a mechanism.
