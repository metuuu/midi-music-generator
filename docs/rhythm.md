...
The catalogue underneath all of it is **19 genres, 389 styles and 72 eras**, and the rhythm section's share is **1,005 drum patterns** — 103 carrying a `cycle`, 184 writing `ghosts`, 10 writing `rolls`. Those are the four numbers on this page most likely to have moved by the time you read it; the count script is four lines over `GENRES` and re-running it is cheaper than trusting them. Everywhere else below, where a number would only record how many styles have opted into something, this page names the mechanism instead — because **absent means no draw** is the property that makes the count uninteresting, and it is the same property in nine separate features.
...
- **`DrumPattern.cycle` is one number for the whole kit.** A figure whose hat cycles against its kick cannot be written.
