...
Jazz also overrides seven individual rules, because the shared table was written from classical voice-leading practice and jazz does not hold all of it. `unresolved-leading-tone` and `chromatic-tone` are disabled outright; `unprepared-dissonance`, `flat-nine`, `augmented-second` and `parallel-perfects` are relaxed; `avoid-fourth` is *tightened*, because the natural 11 over a major seventh is a genuine avoid note here. See [smoothness.md](smoothness.md#do-genres-share-the-rules) for the reasoning and [rules.md](rules.md) for the full table.
