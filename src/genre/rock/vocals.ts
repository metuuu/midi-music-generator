 ...
 * was English. There are five — `sargam` came with the Indian genre after this was
 * written — and it fails here from the far side of the reason `machine` does two
 * paragraphs down: `machine`'s `codaDensity: 0.55` is already too closed, and
 * `sargam` is at 0 with its syllables pinned to scale degrees, which is a note
 * being named rather than a vowel being held. The count was wrong; the answer was
 * not.
 *
 ...
 * available correction without writing a sixth language into a file this genre
