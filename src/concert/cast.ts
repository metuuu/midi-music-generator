...
import type {
  Accessory, Archetype, ArchetypeSpec, Cast, Garment, HairStyle, Look, PartRef,
  Performer, Posture, StageMachine, Station, SynthRigId, Venue,
} from './types.js';
...
/**
 * Hair that is a covered head already, and which therefore takes the hat slot.
 *
 * `hood` and `wrap` are `HairStyle` values because of where they are *drawn* —
 * both need the open shell that leaves the face a hole in the geometry, and
 * `Look` has one hair slot and one accessory list — but neither is hair. They
 * are cloth over the whole skull, and they are worn in exactly the place the
 * seven above are worn. So the sentence the group table is built on, that these
 * are *places on a body and not lists of lookalikes*, says they belong to the
 * hat group; the type they happen to be declared in does not.
 *
 * This is a casting rule and not a rendering one, and the choice is worth the
 * paragraph because the renderer could have handled it. `performer-hair.ts`
 * publishes a `HairProfile` that says a hood is a `garment`, and the arbitration
 * in `performer-accessories.ts` will happily lift a porkpie clear of the hood's
 * dome — it does exactly that on the costume bench, which is where you can see
 * why it is not the answer. A hat perched on an inflated hood is not a
 * photograph of anything.
 *
 * More to the point, refusing it in the renderer would mean the IR still says
 * this player is wearing a porkpie: the `Look` records it, the bench label
 * prints it, `wardrobeFor` counted it against `MAX_ACCESSORIES`, and the only
 * thing that disagrees is the picture. That is precisely the failure the
 * renderer's own header calls out — a wardrobe entry that draws nothing — and
 * the reason there is a `never` assertion in each of its switches. A rule
 * enforced here keeps `Look` honest: what the contract says a player wears is
 * what walks on stage.
 *
 * It is filtered after the fact rather than folded into the loop below, and that
 * is deliberate too. `hairStyle` is drawn from the same stream *after* the
 * accessory draws, so moving it earlier would reshuffle every look in every
 * genre — a whole catalogue of wardrobes re-rolled to fix two pairings. The
 * player simply ends up with one fewer accessory, which is the same thing that
 * happens to anybody whose second hat is refused.
 *
 * `headphones` is not in `HATS`, so it survives this untouched. That is the
 * point of it being left out of the group in the first place: `ambient:sampler`
 * draws a hood and cans together, and a tender at a rig in a hood and cans is as
 * real a photograph as the one in a beanie.
 */
const COVERED: HairStyle[] = ['hood', 'wrap'];
...
function makeLook(args: {
  ...
  // The hat slot again, now that it is known whether the hair took it. See
  // `COVERED`: a hood and a wrap are the same place on the body as a porkpie.
  const worn = COVERED.includes(hairStyle)
   ...
   *
   * **Last of every draw in this function, and skipped entirely when the genre
   * has no table.** Both halves of that sentence are load-bearing and neither is
   * a style preference.
   *
   ...
   * dressing arabic changes arabic's silhouettes and nothing else about arabic —
   * the same faces, the same colours, the same hats, in different clothes. That
   * is the difference between a change you can look at on the bench and a
   * catalogue-wide reshuffle you have to take on trust.
   *
  ...
  for (const layer of LAYER_ORDER) {
    if (layer === 'drums') {
      ...
      if (song.drums.events.length && isPlayedByHand(song.drums.source ?? 'kit')) {
         ...
         *
         * `song.drums` is one event stream with no notion of who plays what,
         * which was the right shape while every voice in it belonged to one
         * kit. It stopped being so when `DrumVoice` grew `lp`/`mp`/`hp`: a
         * darbuka is not a kit piece and its player is not a drummer with a
         * fifth limb, so those three voices were resolving to a patch of air
         * beside the floor tom and a seated drummer was reaching into it.
         *
         * `drumStations` reads the voices actually written and says which
         * objects the part needs. Most numbers still need exactly one and this
         * still drafts exactly one player. An arabic `maqsum` needs only the
         * hand drum, and drafts *no* kit — which is the half of this that
         * matters most, because the alternative was an acoustic kit staged
         * behind a player who never touched it. A funk `congas` needs both, and
         * gets two people, which is what a conga player beside a drummer is.
         *
